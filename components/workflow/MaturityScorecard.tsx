...
export function MaturityScorecard({ steps, ratings, compact }: MaturityScorecardProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Priority Gaps — compact: inline list, full: cards */}
      {gaps.length > 0 && (
        compact ? (
          <div className="mt-4 pt-4 border-t border-emerald-200">
            <p className="text-sm font-medium text-emerald-800 mb-2">Priority Gaps:</p>
            <div className="flex flex-wrap gap-2">
              {gaps.map((step) => (
                <Badge key={step.id} variant="outline" className="bg-red-50 text-red-700 border-red-200">
                  {step.title} — {step.aiOpportunity.impact} impact
                </Badge>
              ))}
            </div>
          </div>
      ...
      {/* Full-mode sections hidden in compact */}
      {!compact && (
        <>
          {/* Impact Estimate */}
